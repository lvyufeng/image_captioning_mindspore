...
def create_dataset(data_path, mode='train', batch_size=32, captions_per_image=5,
    ...
    dataset = dataset.map(cap_transforms,
                          input_columns=["enc_captions"],
                          output_columns=["enc_captions", "all_captions"],
                          column_order=["image", "enc_captions", "caplens", "all_captions"],
                          num_parallel_workers=num_parallel_workers)

    # repeat N times and use batch_map to get different captions
    dataset = dataset.repeat(captions_per_image)
    dataset = dataset.batch(1, input_columns=["image", "enc_captions", "caplens", "all_captions"], per_batch_map=resize)

    dataset = dataset.batch(batch_size, input_columns=["image", "enc_captions", "caplens", "all_captions"], per_batch_map=resize2,
                            drop_remainder=drop_remainder)
    if shuffle:
        dataset = dataset.shuffle(batch_size)

    if mode == 'train':
        dataset = dataset.project(["image", "enc_captions", "caplens"])
    return dataset